...
def test_add_coin_to_tracker_success(client):    
    ...
    coin = "bitcoin"
    response = client.post(f"/tracker/{coin}", headers=headers)

    assert response.status_code == 200 
    assert response.json()["code"] == 200
    assert response.json()["status"] == "success"
    assert response.json()["data"][0]["name"] == "Bitcoin"
...
def test_add_coin_to_tracker_failed_unauthenticated(client):    
    coin = "logam"
    response = client.post(f"/tracker/{coin}")
